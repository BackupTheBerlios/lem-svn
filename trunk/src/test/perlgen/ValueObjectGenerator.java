...
public class ValueObjectGenerator {
	...
	private static void listClasses(Domain domain) throws IOException {
		...
		for (Iterator it = classes.values().iterator(); it.hasNext();) {
			org.jdns.xtuml.metamodel.Class theClass = (org.jdns.xtuml.metamodel.Class) it
					.next();
	...
	private static void buildPackage(org.jdns.xtuml.metamodel.Class theClass,
		...
		/*
		 * 
		 * PrintWriter out = new PrintWriter( outDir.getAbsolutePath() + "/" +
		 * className + ".pm" );
		 * 
		 * out.println( "use Object;"); out.println( "use XMLserV2;");
		 ...
		 * 
		 * out.println( "@ISA = qw( Object XMLserV2 );"); out.println(
		 * "Ser::registerClass( \"" + className +"\" );" );
		 * 
		 * out.println( "\n# Attribute definitions\n");
		 * 
		 * HashMap attributes = theClass.getAttributes(); out.println( "my
		 * %fields = (" ); for ( Iterator it = attributes.values().iterator();
		 * it.hasNext(); ) { Attribute attribute = (Attribute) it.next();
		 * out.print( " " + attribute.getName() + " => undef" ); if (
		 * it.hasNext() ) out.println( "," ); } out.println( "\n);"); //
